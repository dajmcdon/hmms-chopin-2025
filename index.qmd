...
## {background-image="gfx/LoveLost-VPO.png" background-size="contain"}
...
called classical music, he puts down the exact notes that he wants, the exact 
instruments or voices that he wants to play or sing those notes—even the exact 
...
<div class=tc>
<img src="gfx/hey-jude-single.jpeg" height=250>
...
## HMM Inference {auto-animate=true}
...
[We want to estimate]{.tertiary}: $\theta$, $p$, $\{X_k\}$, and $\{S_k\}$
...
1. Track at most $J$ paths through the $|\mathcal{S}|^n$ tree

2. At time $k$, given $J$ paths, propagate each one forward

3. Sample the $\approx J^2$ possibilities to get only $J$
...
ggplot(m, aes(Var1, Var2, fill = value)) + 
  ...
  theme_void() +
  coord_equal() +
  geom_hline(yintercept = sply, color = "#ffffff", linewidth = .2) +
  geom_vline(xintercept = splx, color = "#ffffff", linewidth = .2) +
  theme(legend.position = "none")
```
